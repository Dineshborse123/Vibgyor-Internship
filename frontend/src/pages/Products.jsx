...
const Products = () => {
  ...
  const addToCart = async (productId) => {
    try {
      await API.post('/cart', { product_id: productId, quantity: 1, user_id: user?.id || 3 });
  ...
  const addToWishlist = async (productId) => {
    try {
      await API.post('/wishlist/add', { product_id: productId, user_id: user?.id || 3 });
      setCartMsg('Item added to wishlist! 💖');
      setTimeout(() => setCartMsg(''), 2500);
    } catch (err) {
