...
const Dashboard = () => {
  ...
  const [stats, setStats] = useState({ products: 0, categories: 0, orders: 0, cartItems: 0, leaves: 0, attendanceRate: 0 });
        ...
        if (isAdmin()) {
          try {
            ...
            if (activeStaff.length > 0) {
              attendanceRate = Math.round((attRes.data.length / activeStaff.length) * 100);
            }
          } catch(e) { console.error("Failed to fetch attendance stats"); }
        }
        setStats({
        ...
        <>
          ...
          <div className="stats-grid">
            <div className="stat-card" style={{ cursor: 'pointer', transition: 'transform 0.2s', border: '1px solid transparent' }} 
              ...
              <div><div className="stat-value">{stats.attendanceRate}%</div><div className="stat-label">Employee Attendance</div></div>
            </div>
